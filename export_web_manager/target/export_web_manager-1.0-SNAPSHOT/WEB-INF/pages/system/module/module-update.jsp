            ...
            <div class="panel panel-default">
                ...
                <form id="editForm" action="/system/module/edit.do" method="post">
                    ...
                    <div class="row data-type" style="margin: 0px">
                        ...
                        <div class="col-md-4 data">
                            <select class="form-control" id="parentId" onchange="document.getElementById('parentName').value=this.options[this.selectedIndex].text" name="parentId">
                                <option value="">请选择</option>
                                <c:forEach items="${moduleList}" var="item">
                                    <c:if test="${item.id != module.id && item.ctype != 2}">
...
<script>
    //1.当类型的选项发生改变时，它的上级模块也要改变
    $("[name='ctype']").change(function () {
        //ctype是被选中的类型
        var ctype = $(this).val();
        $.get({
            url:"/system/module/findParentModule.do",
            data:{"ctype":ctype},
            success:function (moduleList) {
                var parentId = "<option value=''>请选择</option>";
                if (moduleList!=null&&moduleList.length>0){
                    for(module of moduleList){
                        parentId += "<option value="+module.id+">"+module.name+"</option>";
                    }
                }
                //将列表加入
    ...
    //2.当上级部门的下拉框被选定时，类型要被确定
    $("#parentId").change(function () {
        //当下拉框被改变时，获取下拉框的ctype，就可以确定类型的选项
        var id = $(this).val();
        $.get({
            url:"/system/module/findCTypeByParentModuleId.do",
            data:{"id":id},
            success:function (result) {
                var ctype = result.ctype;
                $("input[name='ctype'][value='"+ ctype +"']").prop("checked",true);
            },
            error:function () {
                alert("error");
            }
